...
//      the seeding envelope (small morphs) keeps the image right.
...
//      independent sub-pixel information).
...
struct SeedingTests {
    ...
    /// steps come in at ≤ 0.8× the unseeded count (measured ~0.67 on this
    /// fixture; a wide framing dilutes toward 1, which is why the camera
    /// matters).
    ...
    /// 1.2 dB of the never-seeded run against the supersampled golden on
    /// this 128² fixture (the cost comes from seeded rays inheriting last
    /// frame's surface estimate, so accumulation carries slightly less
    /// independent sub-pixel information).
    @Test(.enabled(if: GPU.available))
...
///   THRESHOLD_SEED_BENCH_DEBUG   print raw stats for the first 3 frames
    ...
    func interleavedSeedBench() throws {
        ...
        for round in 0..<rounds {
            for (i, _) in configs.enumerated() {
                ...
                // NOTE plain interpolation everywhere: String(format:) with
                // %d after %s desyncs on this platform, and unapplied
                // `.map(Double.init)` on [UInt64] resolves to
                // Double.init(bitPattern:) — both burned this harness once.
                let medMs = String(format: "%7.3f", Self.median(segment.gpuMs))
                let medSteps = Int(Self.median(segment.steps.map { Double($0) }))
                let medRestarts = Int(Self.median(segment.restarts.map { Double($0) }))
                print("round \(round) \(configs[i].name.padding(toLength: 14, withPad: " ", startingAt: 0)) median \(medMs) ms  steps \(medSteps)  restarts \(medRestarts)")
            }
        }
        ...
        // and only the clean rounds should be quoted).
        ...
        for (i, config) in configs.enumerated() {
            ...
            let roundsStr = roundMedians.map { String(format: "%.2f", $0) }
                .joined(separator: "/")
            print("\(config.name.padding(toLength: 14, withPad: " ", startingAt: 0)) "
                  + "pooled \(String(format: "%7.3f", pooled)) ms  "
                  + "(p25 \(String(format: "%7.3f", p25)), p75 \(String(format: "%7.3f", p75)))  "
                  + "rounds \(roundsStr)  steps \(Int(steps))  "
                  + "restarts \(Int(restarts))  psnr \(String(format: "%5.2f", psnrs[i])) dB")
        ...
        // git-ignored; history.csv stays untouched — its schema belongs to
        // bench-suite.sh).
