...
final class TemporalReconstructor {
    ...
    /// Temporal seeding (phase A2): arm `seedTexture` to hand the march the
    /// resolved history as a warm-start source. Defaults FALSE at the class
    /// level so every existing temporal path (tests, rigs) keeps its exact
    /// phase-A behavior; the visionOS shell arms it from UserDefaults/env
    /// (`threshold.render.reconSeed`, default ON) at loop start.
    ...
    // MARK: Temporal seeding (phase A2)

    /// The history-aux array (r = resolved world t, g = sample count) the
    /// march may warm-start from THIS frame — the same read side the frame's
    /// resolve will consume — or nil when seeding must not run: disabled,
    /// no temporal mode, history invalid or about to reset, the world is
    /// morphing hard (volatility ≥ 0.5 — stale distances would spray restart
    /// taps for nothing), or the history pair is the wrong size (the resolve
    /// will reallocate this frame, so its contents describe a different
    /// grid). Call BEFORE `encodeResolve` — the ping-pong flips there.
    func seedTexture(
        volatility: Float, accWidth: Int, accHeight: Int, slices: Int
    ) -> MTLTexture? {
        guard seedingEnabled, mode != .off, historyValid, !pendingReset,
              volatility < 0.5,
              let aux = historyAux[historyRead],
              aux.width == accWidth, aux.height == accHeight,
              aux.arrayLength == slices
        else { return nil }
        return aux
    }
